...
AFRAME.registerComponent('ghost-piece', {
  ...
  init(this: GhostComponent) {
    ...
    this.el.sceneEl?.addEventListener('block-rotated', this.updateGhost);
    this.calculate();
  },
  remove(this: GhostComponent) {
    this.el.sceneEl?.removeEventListener('block-moved', this.updateGhost);
    this.el.sceneEl?.removeEventListener('block-rotated', this.updateGhost);
  },
  calculate(this: GhostComponent) {
    ...
    const active = this.el.sceneEl?.querySelector('[block-movement]');
    if (!active) return;
    const pos = active.object3D.position;
    mesh.object3D.quaternion.copy(active.object3D.quaternion);
