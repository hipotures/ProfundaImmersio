...
function spawnNewBlock(
  ...
  mappings: { left: { x: string; y: string }; right: { x: string; y: string } }
) {
  ...
  block.setAttribute('block-movement', {
    ...
    axisX: mappings.right.x,
    axisY: mappings.right.y
  });
  ...
  block.setAttribute('block-rotation', {
    ...
    boardDepth: boardDims.depth,
    boardHeight: boardDims.height,
    axisX: mappings.left.x,
    axisY: mappings.left.y
  });
  ...
  const halfD = boardDims.depth / 2;
  const xIdx = Math.round(0 + halfW - 0.5);
  const zIdx = Math.round(0 + halfD - 0.5);
...
export async function startGame(difficulty: string, board: string) {
  console.log('Profunda Immersio starting...');
  try {
    const config = await loadConfig('/config/config.yaml');
    const boardSize = config.boardSizes.find(b => b.name === board) || config.boardSizes[0];
    const mappings = config.controls.joystickMapping;
    ...
    let score = 0;
    const chosen = config.difficultyLevels[difficulty] || config.difficultyLevels.easy;
    let timeLimit = chosen.startTime;
    ...
    const scoreKey = `highScore-${board}-${difficulty}`;
    ...
    scene.addEventListener('block-settled', () => {
      ...
      spawnNewBlock(board, next, '#fff', dims.height, dims, mappings);
    ...
    nextQueue.push(randShape());
    spawnNewBlock(board, first, '#fff', dims.height, dims, mappings);
    queueText.setAttribute('value', 'Next: ' + nextQueue.map(s => s.name).join(', '));
  } catch (err) {
    console.error(err);
  }
